package mvc.animator.controller;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.Map;

import mvc.animator.model.AnimationModelImpl;
import mvc.animator.model.ShapeColor;

/**
 * This class implements the AnimationBuilder interface.
 */
public class AnimationBuilderImpl implements AnimationBuilder<AnimationModelImpl> {
  ...
  public AnimationModelImpl build() {
    ...
    for (HashMap.Entry<String, ShapeEntry> shape : shapeMap.entrySet()) {
      ...
      int xcord = initialParams.x1;
      int ycord = initialParams.y1;
      ...
      model.addShape(shape.getValue().shapeIndex, shape.getKey(), shape.getValue().shapeType, xcord,
              ycord, dimOne, dimTwo, new ShapeColor(redStart, greenStart, blueStart), appear,
              disappear);
