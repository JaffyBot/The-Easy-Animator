...
/**
 * An abstract class that represents animation types (i.e., add a shape, move a shape, change the
 * color of a shape, and resize a shape).
 */
abstract class AnimationTypeAbs {
  ...
  /**
   * A getter function that puts all of the shape string attributes into the values and the shape
  ...
  /**
   * A getter function that puts all of the shape integral/double attributes into the values and the
  ...
  public HashMap<String, Double> getDoubleParameters() {
    ...
    if (this.animationType.equals("addShape")) {
      doubleMap.put("startingX", this.shape.xCord);
      doubleMap.put("startingY", this.shape.yCord);
