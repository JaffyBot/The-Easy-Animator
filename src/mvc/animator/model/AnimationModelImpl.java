...
public class AnimationModelImpl implements AnimationModel {
  ...
  double endTime;
  int[] canvasDimensions; // related method hasn't been implemented yet
      ...
      if (animation.animationType.equalsIgnoreCase("move")) {
        for (int j = (int) animation.fromTime; j < endTime; j++) {
          ...
          if (animationMap.get(identifier).get(j) != null) {
            ...
            animationMap.get(identifier).get(j).xCord = shapeFrame[0];
            animationMap.get(identifier).get(j).yCord = shapeFrame[1];
          }
        }
      }
